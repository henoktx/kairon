...
from services.email.mailersend_service import MailerSendService
from services.email.exceptions import EmailServiceError
...
class TestMailerSendService(TestCase):
    ...
    @patch("services.email.mailersend_service.emails.NewEmail")
    ...
    def test_send_email_api_error(self, mock_new_email):
        ...
        self.assertIn("Erro ao enviar email", str(context.exception))

    @patch("services.email.mailersend_service.emails.NewEmail")
    def test_send_bulk_email_success(self, mock_new_email):
        mock_mailer = MagicMock()
        mock_mailer.send.return_value = 202
        mock_new_email.return_value = mock_mailer

        to_emails = ["test1@test.com", "test2@test.com"]
        expected_recipients = [{"email": email} for email in to_emails]
        ...
        mock_mailer.set_mail_to.assert_called_once_with(expected_recipients, {})
        mock_mailer.set_reply_to.assert_called_once_with(
            {"email": settings.DEFAULT_FROM_EMAIL, "name": settings.DEFAULT_FROM_NAME},
            {},
        )
    ...
    @patch("services.email.mailersend_service.emails.NewEmail")
    def test_send_bulk_email_with_template(self, mock_new_email):
        mock_mailer = MagicMock()
        mock_mailer.send.return_value = 202
        mock_new_email.return_value = mock_mailer
    ...
    @patch("services.email.mailersend_service.emails.NewEmail")
    def test_send_bulk_email_api_error(self, mock_new_email):
        mock_mailer = MagicMock()
        mock_mailer.send.side_effect = Exception("API Error")
        mock_new_email.return_value = mock_mailer
        ...
        with self.assertRaises(EmailServiceError) as context:
            self.service.send_bulk_email(
                to_emails=to_emails,
                subject=self.test_subject,
                content=self.test_content,
            )

        self.assertIn("Erro ao enviar emails em massa", str(context.exception))
    ...
    @patch("services.email.mailersend_service.emails.NewEmail")
    def test_send_bulk_email_non_202_response(self, mock_new_email):
        mock_mailer = MagicMock()
        mock_mailer.send.return_value = 400
        mock_new_email.return_value = mock_mailer
