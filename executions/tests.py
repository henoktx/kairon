...
class ExecutionServicesTestCase(TestCase):
    ...
    def test_start_execution_without_schedule(self):
        with patch('executions.services.start_execution.async_to_sync') as mock_async_to_sync:
            mock_async_to_sync.return_value = lambda *args, **kwargs: "mock_workflow_id_123"
            ...
            self.assertEqual(self.execution_without_schedule.temporal_workflow_id, "mock_workflow_id_123")
            self.assertEqual(self.execution_without_schedule.status, "running")

    def test_start_execution_with_schedule(self):
        with patch('executions.services.start_execution.async_to_sync') as mock_async_to_sync:
            mock_async_to_sync.return_value = lambda *args, **kwargs: "mock_schedule_workflow_id_456"
            ...
            self.assertEqual(self.execution_with_schedule.temporal_workflow_id, "mock_schedule_workflow_id_456")
    ...
    def test_reset_execution_success(self):
        ...
        with patch('executions.services.reset_execution.async_to_sync') as mock_async_to_sync:
            mock_async_to_sync.return_value = lambda *args, **kwargs: set_execution_to_running()
